...
- `DASHBOARD_PERFORMANCE_API_URL`: 대시보드가 읽을 통합 Backend API URL입니다. 응답은 `PerformanceApiResponse` 형태여야 합니다.
- `DASHBOARD_DATA_API_URL`: `DASHBOARD_PERFORMANCE_API_URL`의 대체 이름입니다. 둘 다 있으면 `DASHBOARD_PERFORMANCE_API_URL`을 우선 사용합니다.
- `DASHBOARD_EXTERNAL_TRAFFIC_API_URL`: 팀에서 만든 수집 서버 URL입니다. 응답은 `{ "trafficSessions": { ... } }` 형태입니다.
- `NEXT_PUBLIC_DASHBOARD_REFRESH_MS`: 브라우저에서 `/api/performance`를 다시 읽는 주기입니다. 기본값은 `30000`이며, `0`이면 자동 갱신을 끕니다.
...
대시보드는 DB에 직접 접속하지 않습니다. 실제 운영 흐름은 아래 구조를 기준으로 합니다.

```txt
Lighthouse / LHCI / RUM / 로그
  -> PostgreSQL 또는 ClickHouse
  -> Backend API
  -> 1_dashboard_app /api/performance
  -> Dashboard UI
```

프론트엔드는 항상 내부 API인 `/api/performance`만 호출합니다. DB가 아직 없거나 backend API가 준비되지 않은 경우에는 mock 데이터를 사용하고, backend API URL이 설정되면 같은 UI가 실제 데이터로 자동 전환됩니다.

통합 backend API의 최소 응답 형태:
...
정확한 필드는 `src/shared/lib/types.ts`의 `PerformanceApiResponse`와 `DASHBOARD_DATA_REQUIREMENTS.md`를 기준으로 맞춥니다.
