...
async function main() {
  ...
  // insert one playwright_run for this session
  ...
  // loop through each page and each run
  for (const page of PAGES) {
    ...
    for (let runNumber = 1; runNumber <= 3; runNumber++) {
      ...
      // insert lighthouse_run (metadata + metrics)
      const lhRes = await client.query(
        `INSERT INTO lighthouse_runs
           (playwright_run_id, url, site_type, page_type,
  ...
  // update playwright_run finished_at
  await client.query(
